...
import { describe, test, expect, vi } from 'vitest'; // Import vitest APIs
...
describe('Testing rc/state/action-creators/index.ts', () => {
  test('updateInstalled Should call the dispatch function provided', () => {
    ...
    // stubbing the child function to check execution
    const childFunction = vi.fn(); // Use vi.fn() instead of jest.fn()
    temp(childFunction);
